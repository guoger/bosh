  ...
  describe Jobs::UpdateRelease do
    ...
    describe 'fixing broken packages' do
      ...
      context 'when uploading existing release' do
        ...
        context 'when package in blobstore is broken or missing' do
          before do
            allow(BlobUtil).to receive(:verify_blob).and_return(false)
          end

          it 'uploads source package anew and updates DB entry' do
        ...
        it 'verifies and fixes package' do
          ...
          expect(job).to receive(:fix_package).with(package, File.join(release_dir, 'packages', 'fake-name-1.tgz')).and_return('new-blobstore-id')
          job.perform
        end
      end
      ...
      context 'when re-using existing packages' do
        ...
        it 'fixes existing package and copy blob' do
          expect(BlobUtil).to receive(:verify_blob).with('fake-blobstore-id-1', 'fake-sha-1').and_return(false)
          expect(job).to receive(:fix_package).with(existing_pkg, File.join(release_dir, 'packages', 'fake-name-1.tgz')).and_return('new-blobstore-id-after-fix')
